...
public:
    ...
    void create();
    // 转换
    int translate(short* data, uint8_t *sourceData, int len, int bytes_per_sample, int n_channel, int n_sampleRate);
    // 销毁
    void destroy();

    inline void putSamples(const SAMPLETYPE *samples, uint nSamples) {
        if (mSoundTouch != NULL) {
            mSoundTouch->putSamples(samples,nSamples);
        }
    }

    inline uint receiveSamples(SAMPLETYPE *output, uint maxSamples){
        if (mSoundTouch != NULL) {
            mSoundTouch->receiveSamples(output,maxSamples);
        }
    }
